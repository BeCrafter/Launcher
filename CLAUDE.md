...
- **提权模型与体验修补**（2026-09-17）：维持「每次操作 osascript 提权」，不做特权 helper（签名阻塞 + 通用 root shell 更危险 + 分发已绕过 Gatekeeper；论证见 refactor-plan「提权模态」节，勿重开）。三处修补：①草稿新建 system/daemon agent 补上说明框（原条件 `!s.isDraft` 写反）；②服务「重启」被拒时给提权引导并言明重拉身份变化；④**system scope 的 launchctl 动词改为「先试无提权、被拒才升级」**（实测：user/system 都在用户自己的 `gui/<uid>` 域，动词无需 root；daemon 的 `system` 域才真 EPERM）——**`plist-service` 的文件读写判定不变，勿顺手降权**；渲染层闸口为 `scope !== 'daemon'`。差异见 migration-map 差异 23
