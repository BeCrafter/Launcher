...
- **发布与安装**（详见 `docs/design/distribution.md`）：
  - 出发布产物：`npm run build:app:release`（= `--arch all --release`，产 zip+dmg，命名 `Launcher-<版本>-<架构>.{zip,dmg}`；**两条安装通道只消费 zip，dmg 为额外产物、不作为推荐路径**）；CI 同款在 `.github/workflows/release.yml`，打 tag `v*` 触发并自动把各产物 sha256 写进 Release 正文
...
│   ├── lib/           # 纯函数:cron/sci/plist/classify/ops-bar(5 态表)/svc-override(别名·host·URL 解析)/elevation(Promise API)/ipc-error(main 业务错误上浮)/utils/modules(注册表)/statusbars
