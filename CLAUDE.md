...
- **发布与安装**（详见 `docs/design/distribution.md`）：
  - 出发布产物：`npm run build:app:release`（= `--arch all --release`，产 zip+dmg，命名 `Launcher-<版本>-<架构>.{zip,dmg}`；**两条安装通道只消费 zip，dmg 为额外产物、不作为推荐路径**）；CI 在 `.github/workflows/release.yml`，**分两个 job**（2026-09-23 拆的）：
    - **`release`**（macOS，挂 `r2-publish` 环境；Node 24）：校验版本/预发布 → typecheck/test → 构建 → 算 sha256 → **上传 Cloudflare R2**（`https://repo.iskill.site/launcher/`，另传 `Launcher-latest-<架构>.zip` 别名并带 `no-store`）→ **把 cask 推到 `BeCrafter/homebrew-brew`**（模板 `packaging/homebrew/launcher.rb` 是唯一事实来源，CI 只 sed 替换 version/url/两个 sha256，tap 仓库那份勿手改）→ 发 GitHub Release（备用下载源 + sha256 留档）
    - **`npm-publish`**（Linux，挂 `npm-publish` 环境）：`needs: release`，仅在 release 成功**且非预发布**时跑；只从仓库拷 `packaging/npm/` 四个文件再 publish，不碰构建产物，故用 Linux runner。**拆成独立 job 是硬约束**——GitHub 一个 job 只能挂一个 environment，而 `NPM_TOKEN` 与 R2 凭据分环境授权；`$GITHUB_ENV` 不跨 job，故 `release` 用 outputs 把 version/tag/is_prerelease 传过去
    - secrets 归属：R2 五个 + `TAP_GITHUB_TOKEN`（PAT，`GITHUB_TOKEN` 跨不了仓）在 `r2-publish`；`NPM_TOKEN` 在 `npm-publish`
    - **推 cask 只动本包那一个文件**（CI 只 `git add "$CASK_PATH"` 并断言暂存区无其他文件——tap 会并存多个包的 cask）；`git push` 被拒即发版重跑，**不要强推**
    - ⚠ 「发布」步骤**绝不能给 `target_commitish`**：release.yml 处理的一律是**已存在**的 tag（该参数无用），而实测只要加上就稳定 403 `not accessible by integration`——即便 token 明确带 `Contents: write`、且同一 token 能建 tag 与 draft release（2026-09-23 在 v0.1.1/v0.1.2 各踩一次）。`dev-release.yml` 里它有用（那里的 tag 由 action 自建，commitish 与 tag 天然一致）
  - **触发与重跑（两个反直觉的坑，2026-09-23 逐条踩过）**：网页 Actions 里的 "Run workflow" 按钮**不会出现**——默认分支 `main` 至今是空的（只有 LICENSE + README），而该按钮要求 workflow 存在于默认分支。但**命令行走 API 可以**：`gh workflow run release.yml --ref <分支或tag> -f tag=vX.Y.Z`（`--ref dev` 即用 dev 的 workflow 定义，源码仍按 `-f tag` 检出）。另一条：**rerun 旧 run 用的是那次运行所在 commit 的 workflow 文件**，不会带上后来推的修复；而且**被 skip 的 job 不会随 rerun 复活**——所以「改了 workflow」或「想让 `npm-publish` 跑」都**必须产生新运行**（推新 tag，或上面那条命令）
  ...
  - **通道 C：npm**（`packaging/npm/`）`npx -y @becrafter/launcher`——包里**只有安装器**（cli.mjs + lib.mjs + README，约 16KB），产物仍从 R2 下载。子命令：`install` / `status`(版本检查) / `versions`(列可用版本) / `uninstall`；两条硬约束决定了形态：npm v7+ **没有 uninstall 钩子**（`npm uninstall` 不会清理 `/Applications`，故自带 `uninstall` 子命令）、`ignore-scripts=true` 会让 lifecycle script **静默失效**——所以**绝不在 postinstall 里干活**。不用平台分包（esbuild 那套）：本应用载荷是 255MB 的 `.app`，落进 `node_modules` 会被 Spotlight/LaunchServices 重复注册；对照同 scope 的 `@becrafter/sail-darwin-arm64` 只有 19.8MB。⚠ scoped 包默认 private，`publishConfig.access` 不能省。CI 发布步骤**与 cask 一样跳过预发布**，跑在独立的 `npm-publish` job 上（`NPM_TOKEN` 归 **`npm-publish` 环境**，与 R2 的 `r2-publish` 分开授权，故 `release` job 读不到它）
  - **安装契约（三条通道必须一致，改一侧要同步另一侧）**：R2 根 / `Launcher-[latest|<版本>]-<架构>.zip` 命名 / **架构判据 `uname -m`**（不能用 `process.arch`——那是 Node 二进制自身的架构，x64 Node 跑在 Apple Silicon 上会错装 x64）/ **必须 `ditto -x -k` 解压**（unzip 与 JS zip 库会丢符号链接与扩展属性、破坏 .app 签名）/ `/Applications` 不可写回落 `~/Applications` / `xattr -dr` / 校验 `Contents/MacOS/Launcher`。`scripts/install.sh`（bash）与 `packaging/npm/cli.mjs`（Node）是两份实现，契约写在 `docs/design/distribution.md`，跨文件断言在 `packaging/npm/contract.test.mjs`（+ 本地夹具 HTTP 的 `packaging/npm/fetch.test.mjs`）；改产物名会直接红
  - **版本发现契约（三条通道共用同一个来源，都在 cdn 上）**：**有哪些版本** → `<cdn 根>/versions.txt`（纯文本一行一版 `0.2.0` / `0.3.0-rc.1 pre`，**由 CI 发版时增量维护并上传**，见 release.yml「生成版本清单」；R2 **不支持列对象**，故必须有这个文件）；**latest 指向哪个版本** → `Launcher-latest-<架构>.zip` 的 HEAD 重定向后最终 URL（别名名里没有版本号）。两侧实现：`install.sh` 的 `list_versions`/`r2_latest`（curl + grep/sed，零依赖）与 `packaging/npm/lib.mjs` 的 `fetchReleases`/`parseVersionsFile`/`fetchLatestVersion`/`versionFromDownloadUrl`。入口：`install.sh --list/--check`、CLI `versions [--pre|--json]`/`status`。⚠ 预发布**也写进清单**（列版本是三通道共用能力），「默认装哪个」才由 latest 别名跳过预发布；**取不到 ≠ 没有版本**（分态提示）；**不能拿本 CLI 的版本冒充 latest**（npx 可能拿到缓存的旧 CLI）
  - 用户安装：`curl -fsSL https://raw.githubusercontent.com/BeCrafter/Launcher/dev/scripts/install.sh | bash`——**curl 不设 quarantine，这是绕过 Gatekeeper 的关键**，产物从 R2 拉（`LAUNCHER_R2_BASE` 可换镜像）；或 Homebrew `brew install --cask becrafter/brew/launcher`；或 `npx -y @becrafter/launcher`——⚠ **Homebrew 会主动打 quarantine**（`cask/download.rb` 无条件调用 `Quarantine.cask!`，且已移除 `--no-quarantine`），故 cask 必须靠 `postflight` 调 `xattr -dr` 清标记，模板与维护手册在 `packaging/homebrew/`
...
- **性能与保真修正**（2026-09-20）：① **`ps -p` 慢 5-10s**：macOS 上 `ps -p <pid,...>` 恒定 ~2.5s（实测 5 个 pid 也要 2.6s，与数量无关），agent 列表每次刷新都要给运行中任务补 uptime → 三处调用点（列表 uptime / 状态 tab 的 cpu·mem·lstart / 端口服务发现）全改成**一次 `ps -eo` 全量列举 + JS 按 pid 过滤**（~0.3-0.6s）；「允许登录时加载」的点击→翻面由 3.5-10s 降到 **~0.36s**（`runAgentIntent` 的列表刷新也改为不阻塞调用方，卡片路径去掉重复 load）。② **未建模顶层键不再锁表单**：`MachServices`/`LimitLoadToSessionType`/`Umask` 这类表单从不触碰的键回到 `preservedTopLevelKeys`（只提示一行「保存时原文保留」），不再进 `unsupportedPaths`；锁死的只剩「表单拥有该键/父键却表达不了」（Program+ProgramArguments 同存 / KeepAlive 未知子键 / env 非字符串 / 结构无法增量改写）——真机 33 个 agent 锁表单 **21 → 5**（逐一核对：4 个 Program 同存 + 1 个结构不可扫描）。③ **单行 plist 保存即格式化**：整份 dict 挤在一行的文件（`com.docker.socket.plist`、keystone 占位）没有原文排版可保留，点补丁只会在单行里塞带缩进的换行 → 改为整份按 `xmlIndent` 重排（含表示层构造 `<data>`/`<date>`/`<real>`/注释的文件仍走逐字节补丁，宁可不美化也不丢表示）✅——差异见 migration-map「阶段 1 落地差异」末尾
- **分发通道能力对齐 + 应用认来源**（2026-09-21）：三条安装通道（curl / Homebrew / npm）此前只有 npm 侧能做版本检查与列版本，现全部对齐且**共用同一个数据源**——CI 发版时维护 R2 上的 `versions.txt`（纯文本一行一版 `0.2.0` / `0.3.0-rc.1 pre`，增量合成、幂等、预发布也写入），`install.sh --list/--check` 与 CLI `versions [--pre|--json]`/`status` 都读它；`latest` 指针仍由 `Launcher-latest-<架构>.zip` 的 HEAD 重定向问出。应用侧同步：新增**安装来源探测**（`main/services/install-channel.ts`，brew 记录 / 启动环境变量两条判据，判不出回退 manual），「关于」页显示来源 + 检查更新**按通道给对命令**（brew/npx/curl 三条，i18n 三键）；**应用内「检查更新」也迁到同一份清单**（原打 GitHub `/releases/latest`，与通道会分叉）；顺带修 `update-check` 的预发布比较（`rc.10 > rc.9` 原来是字典序判反）
- **端口服务「列表变白」根因修复 + 刷新闪动收口**（2026-09-23，用户报告「间隔几秒之后直接白屏」，怀疑是 3s 轮询刷新）：**根因不在刷新而在刷新推的内容** —— `scanOnce` 里 `ps -eo` 结果**完全不校验**，而 services 由 `rows.filter(r => psMap.has(r.pid))` 得出，于是 ps 失败/超时（或输出形态不认识）⇒ psMap 空 ⇒ **每一行都被滤掉 ⇒ 空表**；空表 + `dockerAvailable=false` 时视图又用 `!dockerDown` 抑制了空状态 ⇒ **整片白**（lsof 一路有 `last.services` 兜底，ps 没有）。且失败态因 `changed || error !== null` 被每 3s 重推，而 `error` 从不进 payload，UI 恒为 null **完全静默**。修：① ps 先校验（`timedOut`/`code!==0`/有 pid 却一行都解析不出 → **保留上次数据 + 置 error**，不再拿空 psMap 过筛）② lsof 补 `timedOut` 判据（半截表不当完整结果）③ 二者显式 `10s` 超时（不再吃可被调到 3s 的 `cmdTimeout`）④ `error` 进 `ScanResult`/`ServicesListPayload`/diff JSON，改为「内容或错误态变化才推」⑤ renderer 加扫描失败横幅（`svc.scanFailed`/`svc.scanFailedHint`）+ 状态栏 monitor 换「扫描失败」+ 空状态不再被 docker 提示条抑制。**闪动**：CDP 量出稳态轮询**零节点增删**（只 1-4 处文本更新），列表不会被清空重画；真 churn 的两处已修 —— 空组**隐藏而不卸载**（`GroupBlock` 新增 `hidden`，此前卸载会丢折叠状态、数据回来整块铺开）、卡片 key 改 `cardKey()`（`<port>:<command>:<addr>` / `docker:<containerId>`，此前用 `id`=`<pid>:<port>` → 进程一重启就销毁重建，吞掉改名输入）。验收：单测 644 全绿（`process-discovery.test.ts` 补 7 例 + `cardKey` 3 例）、真机以假 `ps` 覆写 PATH 端到端（失败时 22 张卡片全保留 + 横幅 + 状态栏「扫描失败」；恢复后自动补全并撤横幅）✅——差异见 migration-map 第 24 条
- **「安装到 PATH」双形态可用 + 判据改为用户登录 shell 的 PATH**（2026-09-23，用户报告打包版点按钮只得到「没有可写的目录，请手动创建符号链接」）：两个独立缺陷。① **判据选错**：`path-link.ts` 原先用 `process.env.PATH` 选落点，而 Finder/Dock 启动的打包应用只有 `/usr/bin:/bin:/usr/sbin:/sbin`（全 root 所有，一个字节都写不进去，用户机上实测该实例的 PATH 正是如此）⇒ 按钮必然失败；可这件事的目的是让**从用户终端启动的外部 Agent** 找到 `launcher-mcp`，判据本应与用户终端一致。改：`shellPath()` 跑一次 `$SHELL -lic`（哨兵 `__LAUNCHER_PATH__…` 只取 rc 噪音之间的那段；fish 走 `string join`；stdin 断开防交互式 shell 挂住）+ 会话内缓存，探测失败回退 `process.env.PATH`（不猜）；`inspectMcpLink`/`installMcpLink` 默认 PATH 同步改掉（**检查与安装必须同源**，否则会出现「装在 `~/.local/bin` 却报 not found」——顺带修好了 brew cask 装的那条链接的识别）；目录不存在但最近已存在祖先可写时按需 `mkdir -p`——只对 `~/.local/bin` 开口子。② **开发态原本完全不支持**（`mcpScriptPath()` 返空串 ⇒ 界面连按钮都不给）：新增 `main/mcp/dev-shim.ts` 生成转调脚本 `<checkout>/node_modules/.cache/launcher-mcp`（`ELECTRON_RUN_AS_NODE=1 <开发 Electron> <checkout>/out/main/launcher-mcp.js`，chmod 755，stdout 保持干净——提示全走 stderr），**每 checkout 一份 ⇒ 路径即身份**（两个 worktree 不会张冠李戴）、不进包（builder 的 files 只收 `out/**`）、不脏仓库；`stdioCommandOf` 改为「已链接就给短名」两模式一致。真机验证：打包形态 PATH → 修复前失败、修复后落在 `~/.local/bin`；dev 里点一次按钮 → shim + 链接 + `state=linked` + 短命令，且 `~/.local/bin/launcher-mcp` 真能拉起 MCP（`initialize` 握手成功）；单测 656 全绿（path-link 16 例 + dev-shim 5 例）✅——差异见 migration-map 第 14 条（AI 助手页迁移段）
- **启动期白屏修复**（2026-09-23，用户报告「首次安装后第一次启动有一段白屏」）：用户确认的四个事实定性——**全程没见过深色过渡页**、浅色主题、时长「有一定间隔」、通道是 curl/brew/npx 之一（刚整份落盘的 258MB bundle）。① **过渡页在「还没被画出来」时就被删了**：`bootstrap()` 模块求值第一行就 `remove()` 掉 `#boot-splash`，而 `<script type="module">` 的执行可能早于浏览器首帧（打包产物 `<head>` 里 686KB 的 render-blocking CSS 又把「可以首绘」与「脚本可执行」卡在同一时刻）⇒ 首帧落到「`#root` 为空」；改由 React 首次 commit 的 `useLayoutEffect` 摘除（与应用 DOM 同帧，见 `renderer/src/boot/splash.ts`）② **那段空窗在浅色下就是近白**：过渡页与 `html` 底色改为内联在 `<head>` + 走 `prefers-color-scheme`（主进程建窗前已写 `themeSource`，媒体查询拿到的就是生效主题，且不依赖 preload）③ **窗口显示时机无门控**：新增 `main/services/boot-gate.ts`（渲染层报「过渡页已上屏」后留 400ms 宽限、应用 commit 立刻显示、用户提前唤起则挂起、3s watchdog 兜底），`showMainWindow()`/Tray 全部改走它，`ready-to-show` 降级为日志 ④ 顺带修 preload 缺失时的**永久空白页**（死代码分支）、过渡页 8s 失败兜底、重复启动不再白建窗口、浅色骨架对比度 ⑤ 首启不可控的慢（1.9MB JS + 686KB 阻塞 CSS 冷解析、258MB bundle 冷页缓存/系统首轮索引）改由「窗口更早出现 + 过渡页覆盖」吸收：`detectInstallChannel`/`ai.start()` 移出建窗前。验证：单测 669 全绿（`boot-gate.test.ts` / `boot-theme.contract.test.ts`）+ 真机冷启逐帧抓屏（浅/深首帧主色 ≈ `#f6f7fb` / `#0e0e17`、BLANK 帧 0）+ 失败路径显示过渡页与重载提示 ✅——详见 migration-map「启动期白屏修复(2026-09-23)」
...
│   ├── services/      # 执行层:shell-runner/dir-watcher/update-check(R2 版本清单)/install-channel(安装来源探测)/elevation(提权)/launchctl-service+plist-service+brew-agent-service+agent-service(阶段1)/crontab-service/process-discovery/termination/docker-service/docker-path(与 brew-path 同款绝对路径探测)
...
└── shared/            # settings.ts(schema+normalize)/models.ts(领域类型)/ipc.ts(通道契约+事件+InstallChannel/UPGRADE_COMMAND)/url-guard(外链 scheme 白名单)/constants/api
```
