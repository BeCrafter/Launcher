...
24. **端口服务「列表变白」根因修复 + 刷新闪动收口(2026-09-23,用户报告「间隔几秒之后直接白屏,怀疑是 3s 轮询刷新」)**。**根因不在刷新,而在刷新把一份空结果推给了 UI**,且 UI 无从分辨「扫描失败」与「真的没有端口」:
    - **唯一能产出空表的路径**:`scanOnce` 里 `ps -eo pid=,user=,etime=,command=` 的结果**完全不校验**,而 `services = rows.filter(r => psMap.has(r.pid))` —— ps 失败/超时/输出形态不认识 ⇒ psMap 为空 ⇒ **每一行都被滤掉 ⇒ `services: []`**(lsof 一路有 `last.services` 兜底,ps 没有)。空表 + `dockerAvailable === false` 时,`ServicesView` 的 `!dockerDown` 又抑制了空状态 → **整片白**(实测复现:卡片 0、空状态 false、只剩一条 docker 提示,状态栏写「共 0 个端口服务」)。
    - **失败还被反复推送**:`if (changed || error !== null)` 让失败态每 3s 重推同一份空表;且 `error` 从不进 `ServicesListPayload`,UI 侧 `error` 恒为 null。
    - **修法(main)**:① `ps` 结果先校验 —— `timedOut / code !== 0 / (有 pid 却一行都解析不出)` 任一命中即**保留上次数据 + 置 error**(与 lsof 同款兜底),绝不再拿空 psMap 过筛;② `lsof` 补 `timedOut` 判据(超时被杀时 stdout 可能只写到一半,半截表此前会被当完整结果解析);③ 二者显式给 `SCAN_CMD_TIMEOUT_MS = 10s`(不再默认吃 `cmdTimeout`:用户可调到 3s,会把繁忙机器的正常扫描判成失败);④ `error` 进 `ScanResult`/`ServicesListPayload`/diff JSON,推送改为「内容或错误态变化才推」;⑤ `ps` 输出语义未变 —— 少列某个 pid 仍按「进程已死」丢弃该行,只有**整表取不到**才走兜底。**修法(renderer)**:新增扫描失败横幅(`svc.scanFailed` / `svc.scanFailedHint`,黄色 warning 材质同 `.invalid-plist-row`)如实说明「以下为上次成功的结果」;状态栏 `monitor` 由「每 3 秒扫描」换成「扫描失败」(`statusbar.svcScanFailed`);空状态不再被 docker 提示条抑制(扫描失败时不谎称「未发现监听端口」)。
    - **刷新闪动(本轮实测 + 修)**:CDP 埋 MutationObserver 量出一轮轮询的真实 DOM 开销 —— **稳态刷新零节点增删**(`add=0/rem=0/newCards=0`,只有 1-4 处 characterData 文本更新,如 uptime),即列表不会被清空重画,**不存在「每 3s 闪一下」**。真正有 churn 的两处已修:① **组变空 → `GroupBlock` 被卸载 → 折叠状态丢失**,数据回来时整块突然铺开(实测「进程」组折叠后清空再恢复 → `open` 被重置为展开);改为 `GroupBlock` 加 `hidden` 属性,**空组隐藏而不卸载**,折叠状态跨空窗保留。② **卡片 key 用 `PortService.id`(=`<pid>:<port>`)→ 进程一重启就销毁重建**(实测新建 DOM 节点数 1),双击改名中的输入被吞、组内其余卡片跟着重排;改用新增的 `cardKey()`(容器 = `docker:<containerId>`,进程 = `<port>:<command>:<addr>`,跨 PID 稳定;三元组唯一性由 lsof 的 `pid:port:addr` 去重保证)。
    - **验收**:单测 644 全绿(`process-discovery.test.ts` 补 7 例:ps 非 0 退出 / ps 超时 / ps 输出不认识 → 三者均「保留上次数据 + error」,ps 少列 pid 仍按已死丢弃,lsof 超时半截表不解析,失败态不重复推送,payload 带出 error;`svc-override.test.ts` 补 `cardKey` 稳定性与不撞键);真机 CDP 端到端(`/tmp` 里放一个可控的假 `ps` 覆写 PATH —— 失败时:22 张卡片**全部保留** + 横幅 + 状态栏「扫描失败」;恢复时:数据自动补全、横幅自动消失,无需人工操作;previously:0 卡片整片白)。
...
14. **「安装到 PATH」双形态可用 + 判据改为用户登录 shell 的 PATH(2026-09-23,用户报告点按钮只得到「没有可写的目录,请手动创建符号链接」)**:两个独立缺陷。
    - **① 判据选错**:`services/path-link.ts` 原先拿 `process.env.PATH` 选落点 —— Finder/Dock 启动的打包应用 PATH 只有 `/usr/bin:/bin:/usr/sbin:/sbin`,四个目录全是 root 所有、一个字节都写不进去(用户机上实测那个打包实例的 PATH 正是这四个),于是按钮必然失败并把用户打发去手动建链接;可这件事的目的恰恰是让**从用户终端启动的外部 Agent** 能找到 `launcher-mcp`,判据本应与用户终端一致。改法:`shellPath()` 跑一次 `$SHELL -lic`(用 `printf '__LAUNCHER_PATH__%s__LAUNCHER_PATH__'` 夹哨兵,只取两哨兵之间那段 —— rc 会往 stdout 印东西;fish 走 `string join : $PATH`;stdin 必须断开,否则交互式 shell 可能等输入)并**会话内缓存**,探测失败回退 `process.env.PATH`(不拿猜出来的目录写文件);`inspectMcpLink`/`installMcpLink` 默认 PATH **同步改掉** —— 检查与安装必须同源,否则会出现「链接明明装在 `~/.local/bin` 却报 not found」的同类误判(**顺带修好 brew cask 那条链接的识别**:它落在 `/opt/homebrew/bin`,此前应用自身 PATH 里没有)。配套:目录不存在但**最近的已存在祖先可写**时按需 `mkdir -p`,**只对 `~/.local/bin` 开口子**(往别人 PATH 里任意不存在的路径新建目录是越界);失败原因压成单行(toast 是 `white-space: nowrap`,多行会被窗口裁掉,完整路径在弹窗上方本就可复制)。
    - **② 开发态原本完全不支持**:`mcpScriptPath()` 在 dev 返回空串(旧注释的理由是「仓库里那份脚本假设打包布局,链进 PATH 只会造出一条坏命令」),于是界面连按钮都不给、命令恒为「二进制 + out/main 入口」两路径形式。改法:新增 `main/mcp/dev-shim.ts`,生成一份转调 shim 到 **`<checkout>/node_modules/.cache/launcher-mcp`** —— 内容 `ELECTRON_RUN_AS_NODE=1 <开发用 Electron 二进制> <checkout>/out/main/launcher-mcp.js "$@"`(chmod 755;入口缺失时给一句自解释的话并退非零;**stdout 保持干净**——MCP 协议就在 stdout,提示全走 stderr)。落点选这里的三条理由:① **路径即身份**,每个 checkout 一份,于是「链接是否指向本副本」的 realpath 判据对 dev/打包一视同仁,两个 worktree 不会互相认领;② **不进包** —— electron-builder 的 `files` 只收 `out/**` 与 package.json;③ 不脏仓库(node_modules 已在 .gitignore)。写入只发生在用户点按钮时(mcpInfo 仍只读);`stdioCommandOf` 改为「已链接即给短名 `launcher-mcp`」,两个模式一致。**失效自愈**:checkout 移动 / `npm ci` 清掉 node_modules 后链接变悬空,界面显示「悬空」并给出「修复指向」,点一下重建(不静默留一条坏命令)。
    - **验证**:真机以「进程 PATH = 打包形态」复现 —— 修复前命中「没有可写的目录」,修复后落在 `~/.local/bin/launcher-mcp` 且 `inspect` 判 linked;dev 里点一次按钮:`node_modules/.cache/launcher-mcp` 生成 + `~/.local/bin/launcher-mcp` 链接 + `state=linked` + 命令切到短名,并**真跑一次** `~/.local/bin/launcher-mcp` 发 MCP `initialize` → 握手成功返回 serverInfo;单测 `path-link.test.ts` 16 例 + `dev-shim.test.ts` 5 例(含真实 shell 探针、不为任意不存在目录建目录的反例、shim 真机执行与参数透传)。
...
- **一次最多显示 10 个**(`MAX_VISIBLE`);超出时锚点条自身成窗口。**指针移到条的上/下缘即持续滚动窗口**
  (rAF + 边缘 26px 感应区 + **按时间累积**,`EDGE_SPEED` 格/秒)—— 不是"一帧一格":后者在 60Hz 下是 60 格/秒,一顿一顿地窜。
- **两头到没到有区分**:渐隐只在「那一侧窗口外**确实还有**锚点」时才加(`fade-top` / `fade-bottom` 分别控制),
  滚到顶不淡出上端、滚到底不淡出下端。⚠ 为此把"滚到底"特判成"当前锚点 = 最后一条":否则末条消息若还没顶到视口上沿,
  当前锚点会停在倒数第二条、窗口滑不到末尾,底部一直淡出,看起来像"后面还有"。
- **横线长度以「焦点」为中心渐变**:焦点那根最长(18px),按距离指数衰减到 4px。焦点 = 鼠标在条上时跟鼠标、否则跟当前阅读位置 ——
  鼠标划过时整列随之起伏(内联宽度 + CSS 过渡),既好看也是"我在哪"的额外提示。
- **焦点落在横线本身**:悬浮只把这根变长变亮,旁边浮出的是**纯文字**(无底/无边框/无阴影)。
  早先做成胶囊时,横线左侧会堆出一个卡片把视线拽走(用户反馈「焦点应该在横线上,而不是它左侧那个阴影」)。
- **悬浮**:目标消息加柔和高亮环。
- **点击**:平滑滚到该消息(顶部留 12px)。
...
### AI pane / 对话页 第四轮调整(2026-09-22,用户要求)

14. **「模型 ID」与「最近用过」合成一个整体** —— 此前 `.settings-row` 自带的分隔线横在输入框与 chips 之间,
    把一件事读成了两件。现在整条线挪到整个 block(即 chips)下方;组内另加一条**虚线**填掉中间那片空白。
    用虚线不用实线是有意的:**实线是「分组边界」**(上一版就是因此显得割裂),**虚线是「组内分隔」**;
    颜色取 `--border2` 而非 `--border`(后者 0.06/0.08 透明度,做成虚线几乎看不见)。
15. **Key 默认隐藏** —— 第二轮曾按要求改成默认明文,本轮改回默认隐藏(眼睛按钮查看)。眼睛钮仍浮在输入框内部右缘。
16. **新会话顶部加语言来源提示** —— 「对话语言跟随应用设置:简体中文」整条可点,直达 设置 › 常规与外观。
    不这么做用户既不知道助手为什么说这种语言,也不知道去哪改。
17. **状态栏去掉与引擎 chip 重复的「供应商 · 模型」** —— 同一串信息在输入区上方 chip 与底部状态栏各显示一次。
    保留 **chip**(可点击跳配置,信息也更靠近输入),状态栏摘要退回子系统状态行(`ai.sb.engine` / `engineOff`)。
18. **锚点区样式**:位置从「整窗正中」提到中间偏上(实测整窗 42.6%);悬浮**去掉底色与阴影**只提亮(否则一列小横杠变成"控件盒子",反而更抢眼);
    **窗口化时上下两端做 mask 渐隐**,配合两端锚点收短,"更远处还有内容"的感觉是靠虚实层次给的,不靠箭头或截断。
    ⚠ 位置百分比是相对 `.ai-main` 的 —— `.ai-main` 从顶栏之下才开始、到输入区结束,所以 `top:42%` 落到整窗其实是 48%(≈正中),取 36% 才真的偏上。
