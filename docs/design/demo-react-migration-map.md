...
③ **会话栏头部重做** —— 原先头部只有一枚 9.5px 暗灰「会话」+ 22px 描边小钮,中间整条空着(用户:「搜索框上面的部分感觉有些空当当」)。改为:20px **身份砖**(`--accent-wash-grad` 底 + 紫描边 + 魔法棒图标,与消息里的 `.ai-bot-avatar` 同源材质)+ 11.5px 白色半粗标题(去掉原 uppercase / 字距)+ **会话数 pill**(`--wash-accent` 底 / `--accent2` 字 / `margin-left:auto` 右对齐,把头部右侧收成"计数 + 折叠"一组)+ 折叠钮缩成 24px **无边框 ghost**(悬停 `--hover` 底 + `--accent2`,不再描边)。头部左右内边距与 `.ai-rail-tools` 的 margin 对齐,使身份砖左缘齐搜索框左缘、折叠钮右缘齐 `+` 右缘。计数由 `aiRenderRail()` 经 `#aiRailCount` 同步(新建 live 会话即 +1)。
